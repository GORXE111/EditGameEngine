...
const std::unordered_map<std::string, Pair>& table() {
    static const std::unordered_map<std::string, Pair> t = {
        ...
        {"palette.title",        {"新增节点",        "Add Node"}},
        {"palette.flow",         {"流程控制",        "Flow"}},
        {"palette.var",          {"变量",            "Variables"}},
        {"palette.action",       {"动作",            "Actions"}},
        {"palette.sense",        {"感知",            "Sensors"}},
        {"palette.op",           {"运算",            "Math / Logic"}},
        {"palette.lit",          {"字面量",          "Literals"}},
        {"palette.list",         {"列表",            "Lists"}},
        {"hint.bp_edit_full",    {"右键空白处新增节点；拖针连线；选中按 Delete 删除；改完点 应用 运行。",
                                  "Right-click empty area to add a node; drag pin→pin to wire; select and Delete; click Apply to run."}},
